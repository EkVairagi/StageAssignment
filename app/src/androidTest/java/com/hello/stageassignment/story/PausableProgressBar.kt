...
internal class PausableProgressBar @JvmOverloads constructor(
    ...
    fun startProgress() {
        Log.d("PausableProgressBar", "Starting progress")
        binding.maxProgress.visibility = GONE
        animation = PausableScaleAnimation(
            0f, 1f, 1f, 1f,
            Animation.ABSOLUTE, 0f,
            ...
            setAnimationListener(object : Animation.AnimationListener {
                override fun onAnimationStart(animation: Animation) {
                    Log.d("PausableProgressBar", "Animation started")
                    binding.frontProgress.visibility = VISIBLE
                    callback?.onStartProgress()
                }
                ...
                override fun onAnimationEnd(animation: Animation) {
                    Log.d("PausableProgressBar", "Animation ended")
                    callback?.onFinishProgress()
                }
            })
            fillAfter = true
    ...
    fun pauseProgress() {
        Log.d("PausableProgressBar", "Pausing progress")
        animation?.pause()
    }

    fun resumeProgress() {
        Log.d("PausableProgressBar", "Resuming progress")
        animation?.resume()
    }

    fun clear() {
        Log.d("PausableProgressBar", "Clearing progress")
        animation?.apply {
            setAnimationListener(null)
            cancel()
            animation = null
    ...
    private inner class PausableScaleAnimation(
        ...
        fun pause() {
            if (mPaused) return
            Log.d("PausableProgressBar", "Animation paused")
            mElapsedAtPause = 0
            mPaused = true
        }

        fun resume() {
            Log.d("PausableProgressBar", "Animation resumed")
            mPaused = false
        }
    }
